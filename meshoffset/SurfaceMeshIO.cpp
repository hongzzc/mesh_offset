...
{
public:
    CmpVec(Scalar eps = std::numeric_limits<Scalar>::min()) : eps_(eps) {}//scalar=float
...
{
    char line[100], *c;//定义了一个长度为100的字符串数组，以及一个字符指针c
    unsigned int i, nT(0);//定义了两个无符号的整数i,以及一个初始值为零的整数nT
    vec3 p;//vec3数据类型实在MatVec.h中定义的一个数据类型，MatVec.h是基于Eigen库所编写的一个头文件
    Vertex v;//定义了顶点元素v，实际上是一个索引
    std::vector<Vertex> vertices(3);//定义了3个顶点元素
    size_t n_items(0);//定义了一size_t类型的参数并赋值为0

    CmpVec comp(std::numeric_limits<Scalar>::min());//std::numeric_limits<Scalar>::min()无限趋于零的一个数
    std::map<vec3, Vertex, CmpVec> vMap(comp);//这个map可以这样理解：vec3是一个key,vertex是一个value,把key值按照从大到小进行排序
    std::map<vec3, Vertex, CmpVec>::iterator vMapIt;//map的一个迭代器

    // open file (in ASCII mode)
    FILE* in = fopen(filename_.c_str(), "r");//FILE *fopen(char *filename, char *mode)此处是以只读的方式打开文件
    if (!in)
        return false;//这里使用的是c风格的读入数据的形式，实际上的C++_语言并不是这样进行操作的

    // ASCII or binary STL?
    c = fgets(line, 6, in);
    PMP_ASSERT(c != nullptr);
    const bool binary =
        ((strncmp(line, "SOLID", 5) != 0) && (strncmp(line, "solid", 5) != 0));//此处的操作不必说
