...
{
public:
    //! default constructor with invalid（无效） index
...
public:
    ...
    private:
        const SurfaceMesh* mesh_;
        Halfedge halfedge_;
        bool is_active_; // helper for C++11 range-based for-loops基于范围的for循环
    };
    ...
    public:
            ...
            : mesh_(m), is_active_(true)
        {
            ...
            {
                halfedge_ = mesh_->halfedge(v);
                if (halfedge_.is_valid() && mesh_->is_boundary(halfedge_))//检查是否半边表中的半边是否是边界半边（边界半边无左接面），如果是的话，那么跳到下一个半边
                    operator++();
            }
        }
    ...
    //! halfedges.
    inline Edge edge(Halfedge h) const { return Edge(h.idx() >> 1); }//此处的>> 是右移一位的意思
...
private:
    ...
    Vertex new_vertex()
    {
        ...
        vprops_.push_back();//点组中加入了一个元素，size+1了！
