...
SurfaceMesh::SurfaceMesh()
{
    ...
    // allocate standard properties添加一些标准的属性
    // same list is used in operator=() and assign()
    vpoint_ = add_vertex_property<Point>("v:point");//点的三坐标
    vconn_ = add_vertex_property<VertexConnectivity>("v:connectivity");//点的外出半边
    hconn_ = add_halfedge_property<HalfedgeConnectivity>("h:connectivity");//给半边添加拓扑属性
    fconn_ = add_face_property<FaceConnectivity>("f:connectivity");//给面添加拓扑属性

    vdeleted_ = add_vertex_property<bool>("v:deleted", false);//初始化点的状态属性，无待删除点
    edeleted_ = add_edge_property<bool>("e:deleted", false);//初始话边的状态，无待删除边
    fdeleted_ = add_face_property<bool>("f:deleted", false);//初始化面的状态，无待删除的面

    deleted_vertices_ = 0;
    deleted_edges_ = 0;
    deleted_faces_ = 0;//待删除的点，边，面的数量都是0
    has_garbage_ = false;//网格中状态为无垃圾
}
...
SurfaceMesh& SurfaceMesh::operator=(const SurfaceMesh& rhs)
{
    if (this != &rhs)
    {
        ...
        hprops_ = rhs.hprops_;
        eprops_ = rhs.eprops_;
        fprops_ = rhs.fprops_;//这五个显然是核心的成员！
...
{
    Vertex v = new_vertex();//实际上是增加了一个索引
    if (v.is_valid())
        vpoint_[v] = p;//只是修改点的属性数组中的三坐标数组
    return v;
}
...
Face SurfaceMesh::add_face(const std::vector<Vertex>& vertices)
{
    ...
    std::vector<bool>& isNew = add_face_is_new_;//这里的isNew是对半边而言的
    ...
    for (i = 0, ii = 1; i < n; ++i, ++ii, ii %= n)
    {
        ...
        //这个应该是很好理解的，非边界的顶点是无法作为新添加的面的顶点的！
    ...
    /*for (i = 0; i < n; ++i)
    {
        size_t m = SurfaceMesh::valence(vertices[i]);
        if (m > 10)
            printf("%d\t", m);
    }*/
